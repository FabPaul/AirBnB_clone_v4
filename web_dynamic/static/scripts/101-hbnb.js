...
<h2>Reviews</h2>
<span data-id="${place.id}" class="review-toggle">show</span>
</div>
<div id="reviews-of-${place.id}">
</div>
</article>`;
      }));
    }
...
<div id="reviews-of-${place.id}"></div>
</div>
</article>`;
        }));
      }
  ...
  $(document).on('click', '.review-toggle', function () {
    ...
    const reviewsDiv = $(`#reviews-of-${placeId}`);
    if (reviewsDiv.is(':empty')) {
      // Reviews are hidden, so show them and change text to "hide"
      $(this).text('hide');
...
<li>
<p>${review.text}</p>
</li> `).appendTo('#reviews-of-' + placeId);
        });
      });
    } else {
      // Reviews are shown, so hide them and change text to "show"
      $(this).text('show');
      reviewsDiv.empty();
    }
  });
